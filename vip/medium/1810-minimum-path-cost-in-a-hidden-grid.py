...
有一个机器人存在于网格中，你需要通过不断尝试使他从初始单元到达目标单元。网格的规格为m x n，并且每个单元的属性值要不为空，要不已被占用。
题目保证初始网格和目标网格不同且均为空。
...
int move(char direction) 沿该方向移动机器人，并返回移动到该单元的消耗值。如果此移动将机器人移动到被占有的单元格或离开网格，
则移动将被忽略，机器人将保持在相同的位置，函数将返回-1。
