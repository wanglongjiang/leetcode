...
'''
思路：双指针
分别由2个指针pa,pb指向a和b，同时开始遍历，当pa或pb为null时，指向另外一个指针开始遍历。当pa=pb时，返回值即为相交的节点。
当a、b相交时，设a的长度为a+c，b的长度为b+c，经过一次交换后到相交节点时2个指针都走了a+b+c的距离，正好相交。
当不相交时，2个指针最终相同的时候正好都在终点。
...
s = Solution()
print(s.getIntersectionNode(fromList([4, 1, 8, 4, 5]), fromList([5, 0, 1, 8, 4, 5])))
