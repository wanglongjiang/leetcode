...
时间复杂度：O(mn),m=cpdomains.length,n=cpdomains[i].length
空间复杂度：O(mn)
'''
    ...
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            ...
            for i in range(len(cpdomain)):
                if cpdomain[i] == '.':
                    counter[cpdomain[i + 1:]] += count
                elif cpdomain[i] == ' ':
                    count = int(cpdomain[:i])
                    counter[cpdomain[i + 1:]] += count
        return [str(count) + ' ' + domain for domain, count in counter.items()]
