...
'''
思路：二分查找
要想价值最大化，需要从序号最高的球开始取。
设从第x个开始取，那么：
当inventory[i]>x时，用inventory[i]-x得到这种颜色的球贡献的球数量，用等差数列求和公式，计算这种颜色的球贡献的分数。
x可以取值的范围是1..max(inventory)，对x进行二分查找直至得到的球数满足orders。

时间复杂度：O(n*log(max(inventory)))
'''
