...
'''
思路：树
将继承关系表示为以国王为根节点的树，继承顺序为：国王->年长子女->年长子女的后代->次子->次子的后代...
树是n叉树，子女是其子节点。继承顺序是中序遍历。
新出生人加入指定节点的子女节点中
如果有人死亡，将其标注为死亡。

时间复杂度：birth为O(1)，death为O(1)，getInheritanceOrder为O(n)
'''
...
class ThroneInheritance:
    def __init__(self, kingName: str):
        self.allHeir = {}
        self.king = Heir(kingName)
        self.allHeir[kingName] = self.king

    def birth(self, parentName: str, childName: str) -> None:
        parent = self.allHeir[parentName]
        child = Heir(childName, parent)
        parent.childrens.append(child)
        self.allHeir[childName] = child

    def death(self, name: str) -> None:
        this = self.allHeir[name]
        this.dead = True
