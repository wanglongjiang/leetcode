...
那么我们需要将一些正号变为负号，使得最后求和的值等于 target。当前比 target 多出了 delta = S - target，那么我们需要在 1, 2, ..., k 中找到若干个数变成负号，
并且它们的和为 delta / 2。可以证明一定能找到和为 delta / 2 的若干个数：如果 delta / 2 <= k，那么只需要选出 delta / 2 即可；如果 delta / 2 > k，那么先选出 k，
再从 1, 2, 3, ..., k - 1 中选出若干个数使得它们的和为 delta / 2 - k，这样就把原问题变成了一个规模更小的子问题。显然 delta / 2 <= 1 + 2 + ... + k，因此一定能选出若干个数。

上面只适用于 delta 为偶数的情况。若 delta 为奇数，则 delta / 2 不为整数，因此无法选出。此时我们只能考虑 k + 1 和 k + 2，求出 1 到 k + 1 的和以及 1 到 k + 2 的和，
它们中必有一个和 1 到 k 的和的奇偶性不同，此时 delta 变为偶数，可以选出若干个数。
