'''
2105. 给植物浇水 II
Alice 和 Bob 打算给花园里的 n 株植物浇水。植物排成一行，从左到右进行标记，编号从 0 到 n - 1 。其中，
第 i 株植物的位置是 x = i 。
...
class Solution:
    def minimumRefill(self, plants: List[int], capacityA: int, capacityB: int) -> int:
        ...
        for i in range(n // 2):  # Alice浇水
            if a < plants[i]:
                a = capacityA
                ans += 1
            a -= plants[i]
        for i in range(n // 2):  # Bob从右向左浇水
            if b < plants[n - 1 - i]:
                b = capacityB
                ans += 1
            b -= plants[n - 1 - i]
        if n % 2:  # 奇数棵花，中间的需要判断谁浇
            if a >= b:
                if a < plants[n // 2]:
                    ans += 1
            else:
...
s = Solution()
print(s.minimumRefill(plants=[2, 2, 3, 3], capacityA=5, capacityB=5))
print(s.minimumRefill(plants=[2, 2, 3, 3], capacityA=3, capacityB=4))
print(s.minimumRefill(plants=[5], capacityA=10, capacityB=8))
print(s.minimumRefill(plants=[1, 2, 4, 4, 5], capacityA=6, capacityB=5))
print(s.minimumRefill(plants=[2, 2, 5, 2, 2], capacityA=5, capacityB=5))
