...
'''
思路：图
如果一个节点i被k条边指向，我们称节点i的入度为k。入度为0的节点肯定无法从其他节点到达，所以这个题目只需要把入度为0的所有节点找出来就可以。
具体算法：
1. 设一个数组d，存储所有节点入度。遍历edges，对于每条边edge[from, to]，把节点to的入度+1
2. 遍历数组d，将其中为0的下标输出到结果list中
...
class Solution:
    def findSmallestSetOfVertices(self, n: int, edges: List[List[int]]) -> List[int]:
        d = [0] * n
        for edge in edges:  # 统计入度
            d[edge[1]] += 1
        ans = []
        for i in range(n):  # 收集结果
            if d[i] == 0:
                ans.append(i)
        return ans
