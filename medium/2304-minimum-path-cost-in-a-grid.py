...
'''
思路：动态规划
设二维数组dp[m][n]，dp[i][j]的意思是移动到i,j的最小代价
那么状态转移方程为：dp[i][j] = grid[i][j]+min(dp[i-1][k]+moveCost[grid[i-1][k]][j])，k=0..n-1
意思是dp[i][j]的最小代价为上一行的最小（代价+移动到本单元格的成本）
...
class Solution:
    def minPathCost(self, grid: List[List[int]], moveCost: List[List[int]]) -> int:
        ...
        for i in range(1, m):
            for j in range(n):
                dp[i][j] += min([v + moveCost[grid[i - 1][k]][j] for k, v in enumerate(dp[i - 1])])
        return min(dp[-1])
