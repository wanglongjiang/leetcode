...
'''
思路：排序 贪心
2个数组都从小到大排序，运动员从最小的开始遍历，依次寻找最小能满足他的训练师
...
class Solution:
    def matchPlayersAndTrainers(self, players: List[int], trainers: List[int]) -> int:
        players.sort()
        trainers.sort()
        m, n, j = len(players), len(trainers), 0
        for i in range(m):
            while j < n and players[i] > trainers[j]:
                j += 1
            if j < n and players[i] <= trainers[j]:
                j += 1  # 找到最小的训练师，将其占用
            else:
                return i  # 运动员已经找不到能够匹配的训练师，返回已经匹配完的运动员人数
        return i + 1  # 运动员全部匹配完，数量是最大索引+1


s = Solution()
print(s.matchPlayersAndTrainers([1], [1]))
