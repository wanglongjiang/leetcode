...
'''
思路：哈希
将每个人的好友列表转成key为编号，val为索引的哈希表。
然后用2重循环找到任意2个对子，然后4名成员两两组合判断是否有不开心
...
class Solution:
    def unhappyFriends(self, n: int, preferences: List[List[int]], pairs: List[List[int]]) -> int:
        ...
        for i, li in enumerate(preferences):
            for idx, no in enumerate(li):
                prefs[i][no] = idx
        unhappys = set()
        for i in range(n // 2):
            for j in range(i):
                x, y, u, v = pairs[i][0], pairs[i][1], pairs[j][0], pairs[j][1]
                if prefs[x][u] < prefs[x][y] and prefs[u][x] < prefs[u][v]:
                    unhappys.add(x)
                    unhappys.add(u)
                if prefs[x][v] < prefs[x][y] and prefs[v][x] < prefs[v][u]:
                    unhappys.add(x)
                    unhappys.add(v)
                if prefs[y][u] < prefs[y][x] and prefs[u][y] < prefs[u][v]:
                    unhappys.add(y)
                    unhappys.add(u)
                if prefs[y][v] < prefs[y][x] and prefs[v][y] < prefs[v][u]:
                    unhappys.add(y)
                    unhappys.add(v)
        return len(unhappys)
