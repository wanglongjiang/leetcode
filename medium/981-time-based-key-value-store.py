...
输入：inputs = ["TimeMap","set","get","get","set","get","get"], inputs = [[],["foo","bar",1],
...
主要的数据容器是一个哈希表，
> key为set和get的入参key，
> value为一个pair，内部是2个同样长度的数组：数组0为value，数组1为timestamp

时间复杂度：单个key长度为m，单次set时间复杂度为O(1)，get时间复杂度为O(logm)
空间复杂度：O(n)
'''
...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        ...
        pair = self.data[key]
        pair[0].append(value)
        pair[1].append(timestamp)

    def get(self, key: str, timestamp: int) -> str:
        if key not in self.data:
            return ''
        pair = self.data[key]
        i = bisect.bisect_right(pair[1], timestamp)
        if i == 0:
            return ''
        return pair[0][i - 1]
