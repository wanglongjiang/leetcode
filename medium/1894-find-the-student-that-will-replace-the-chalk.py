...
'''
思路：前缀和 二分查找
求前缀和数组s，每轮需要消耗的粉笔数量就是s[n-1]，
k mod s[n-1]为消耗了x轮之后还需要消耗的粉笔数，这个时候在前缀和数组里面二分查找，找到的索引即为需要补充的学生编号
...
class Solution:
    def chalkReplacer(self, chalk: List[int], k: int) -> int:
        s = [0] * len(chalk)
        s[0] = chalk[0]
        for i in range(1, len(chalk)):  # 计算前缀和数组
            s[i] = s[i - 1] + chalk[i]
        return bisect.bisect_right(s, k % s[len(chalk) - 1])  # 在前缀和数组里面搜索最后一轮需要消耗的量
