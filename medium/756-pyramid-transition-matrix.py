...
1、遍历allowed，构造哈希表，key为前2个字符，value为这2个字符顶上的字符list
2、从bottom出发，用回溯生成其上一层的所有字符组合，加入bfs的队列
3、如果能够进行到最上面一层，则返回true
...
class Solution:
    def pyramidTransition(self, bottom: str, allowed: List[str]) -> bool:
        # 1. 构造哈希表
        allowedTab = defaultdict(list)
        for word in allowed:
            allowedTab[word[:2]].append(word[2:])

        # 定义回溯函数，该函数输入为一层的字符备选，输出为一层的合法字符组合

        def bt(charsList: List[list], preWords: set, i: int):
            if i == 0:
                preWords.update(charsList)
            else:
                pass
