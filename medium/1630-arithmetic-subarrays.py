...
'''
思路：哈希
遍历left,right数组对于每一个查询left[i],right[i]，
1. 将nums[left[i]]..nums[right[i]]加入哈希表，
2. 然后等差：diff=(子数组最大值-子数组最小值)/(子数组大小-1)
3. 从子数组最小值开始，每次累加diff，判断是否在哈希表中存在
...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], left: List[int], right: List[int]) -> List[bool]:
        ...
        for i in range(m):
            hashset = set()
            submax, submin = float('-inf'), float('inf')
            for num in nums[left[i]:right[i] + 1]:  # 将nums[left[i]]..nums[right[i]]加入哈希表，找到最大最小值
                hashset.add(num)
                submax = max(submax, num)
                submin = min(submin, num)
            diff = (submax - submin) / (right[i] - left[i])  # 计算出等差值
            val = submin + diff
            for j in range(right[i] - left[i]):  # 从最小值开始，依次累加diff，然后查看哈希表中是否存在
