...
思路：模拟
从左到右依次遍历每个球：
当前所在单元格=1时，尝试向右移动，如果右边为边界或者右边为-1，则无法移动；如果右边为1，移动成功，到下一行继续移动。
当前所在单元格=-1时，尝试向左移动，如果左边为边界或者左边为1，无法移动；如果左边为-1，横向移动成功，到下一行继续移动。
...
class Solution:
    def findBall(self, grid: List[List[int]]) -> List[int]:
        ...
        for j in range(n):
            ...
            for i in range(m):
                if grid[i][j] == 1:
                    if j == n - 1 or grid[i][j + 1] == -1:  # 无法向右移动，中断
                        break
                    j += 1  # 向右移动
                else:
                    if j == 0 or grid[i][j - 1] == 1:  # 无法向左移动，中断
                        break
                    j -= 1  # 向左移动
            else:  # 能够移动到底部，将出口索引写入ans
                ans[col] = j
        return ans
