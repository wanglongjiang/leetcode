...
'''
思路：BFS
如果一个数据服务器i与主服务器的距离为n，那么它发送出的第1个数据包会在第d秒到达主服务器，
第2*d秒回到自身，在此期间发送了packNum = (2 * d - 1) // patience[nextnode] + 1个数据包，
最后一个数据包的发送时间是(packNum - 1) * patience[nextnode] ，它又经过2d+1的时间到达终点。
        ...
        while q:
            ...
            for nextnode in g[node]:
                if not marked[nextnode]:
                    packNum = (2 * d - 1) // patience[nextnode] + 1  # 计算共发出了多少个数据包
                    ans = max(ans, (packNum - 1) * patience[nextnode] + 2 * d + 1)
...
print(s.networkBecomesIdle(edges=[[0, 1], [0, 2], [1, 2]], patience=[0, 10, 10]))
