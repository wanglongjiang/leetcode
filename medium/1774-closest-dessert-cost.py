...
'''
思路：枚举
枚举2m种配料的所有组合，然后与n种基料进行组合，找到最接近target的
...
class Solution:
    def closestCost(self, baseCosts: List[int], toppingCosts: List[int], target: int) -> int:
        ...
        tcom = set([0])
        for i in range(1, len(toppingCosts) + 1):
            for c in combinations(toppingCosts, i):
                tcom.add(sum(c))
        ans = inf
        for baseCost in baseCosts:
            for c in tcom:
                if abs(baseCost + c - target) < abs(ans - target):
                    ans = baseCost + c
                elif abs(baseCost + c - target) == abs(ans - target) and baseCost + c < ans:
                    ans = baseCost + c
        return ans


s = Solution()
print(s.closestCost([10, 1, 10], [7, 5, 1, 1, 1], 5))
