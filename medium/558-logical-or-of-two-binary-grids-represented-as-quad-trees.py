...
'''
思路：分治
如果2个节点都不是叶子节点，则分治处理4个子节点
如果有1个节点是叶子节点，且叶子节点值为1，则返回值为1的叶子节点
如果有1个节点是叶子节点，且叶子节点值为0，则返回另外一个非叶子节点
如果2个节点都是叶子节点，返回值的或
...
class Solution:
    def intersect(self, quadTree1: 'Node', quadTree2: 'Node') -> 'Node':
        if quadTree1.isLeaf and quadTree2.isLeaf:
            quadTree1.val = 1 if quadTree1.val or quadTree2.val else 0
            return quadTree1
        if not quadTree1.isLeaf and not quadTree2.isLeaf:
            ...
            return quadTree1
        leaf, other = quadTree1 if quadTree1.isLeaf else quadTree2, quadTree1 if not quadTree1.isLeaf else quadTree2
        if leaf.val:
            return leaf
        else:
            return other
