...
'''
思路：矩阵
1. 第1次遍历矩阵，求每行、每列的最大值，保存到maxCols,maxRows中
2. 第2次遍历矩阵，将grid[i][j]设置为min(maxCols[j], maxRows[i])-grid[i][j]
...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        n = len(grid)
        maxCols, maxRows = [float('-inf')] * n, [float('-inf')] * n
        for i in range(n):
            for j in range(n):
        ...
        for i in range(n):
            for j in range(n):
                ans += min(maxCols[j], maxRows[i]) - grid[i][j]
        return ans
