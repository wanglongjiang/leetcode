...
'''
思路：哈希
将商品和价格加入哈希表pricesMap,key为products[i],value为prices[i]
执行getBill时，累计product[i]的价格*amount的和money+=self.pricesMap[product[i]] * amount[i]，
然后如果是第n个顾客，需要打折money - (discount * money) / 100

时间复杂度：每次getBill为O(m),m=len(product)
'''
...
class Cashier:
    def __init__(self, n: int, discount: int, products: List[int], prices: List[int]):
        ...
        for product, price in zip(products, prices):
            self.pricesMap[product] = price

    def getBill(self, product: List[int], amount: List[int]) -> float:
        self.count += 1
        money = 0
        for p, a in zip(product, amount):
            money += self.pricesMap[p] * a
        if self.count % self.n:
            return money
        return money - (self.discount * money) / 100
