...
'''
思路：哈希表 有序集合
设一个哈希表，用于保存index->number的映射
设一个哈希表，key为number，value为有序集合保存[index]
...
class NumberContainers:
    ...
    def change(self, index: int, number: int) -> None:
        ...
        if number not in self.num2idx:
            self.num2idx[number] = SortedList()
        self.num2idx[number].add(index)

    def find(self, number: int) -> int:
        if number in self.num2idx and self.num2idx[number]:
